"""Collaborators (software contributors and co-authors): who they are and where else they appear."""
...
def update_collaborators(cv: dict, refresh: bool = False) -> None:
    ...
    tracked_citations = {
        ...
        for item in [*cv.get("libraries", []), *cv.get("contributions", []), *cv.get("personal_tools", [])]
        if (repo := github_repo(item.get("repository")))
    }
    ...
    for qid, rows in (wikidata_statements(stale) if stale else {}).items():
    ...
    (DATA_DIR / "collaborators.yml").write_text(
